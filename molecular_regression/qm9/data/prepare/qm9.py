...
def gen_splits_gdb9(gdb9dir, cleanup=True):
    ...
    Ntrain = int(0.3*Nmols)
    Ntest = int(0.1*Nmols)
    Nvalid = int(0.1*Nmols)  
    Nstrain = Nmols - (Ntrain + Ntest + Nvalid)
    # Ntrain = int(0.2*Nmols)
    # Ntest = int(0.05*Nmols)
    # Nvalid = int(0.05*Nmols)  
    # Nstrain = Nmols - (Ntrain + Ntest + Nvalid)

    print('Ntrain', Ntrain)
    print('Ntest', Ntest)
    print('Nvalid', Nvalid)
    print('Nstrain', Nstrain)
    ...
    train, valid, test, strain, extra = np.split(
        data_perm, [Ntrain, Ntrain+Nvalid, Ntrain+Nvalid+Ntest, Ntrain+Nvalid+Ntest+Nstrain])

    assert(len(extra) == 0), 'Split was inexact {} {} {} {}'.format(
        len(train), len(valid), len(test), len(strain), len(extra))

    train = included_idxs[train]
    valid = included_idxs[valid]
    test = included_idxs[test]
    strain = included_idxs[strain]

    splits = {'train': train, 'valid': valid, 'test': test, 'strain': strain}
