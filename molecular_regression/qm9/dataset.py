...
from qm9.data.utils import initialize_strain_datasets
...
def retrieve_strain_dataloaders(batch_size, new_strain_dict, prop, num_workers=1):
    # Initialize dataloader
    args = init_argparse('qm9')
    args, datasets, num_species, charge_scale = initialize_strain_datasets(args, args.datadir, 'qm9', new_strain_dict, prop,
                                                                    subtract_thermo=args.subtract_thermo,
                                                                    force_download=args.force_download
                                                                    )
    qm9_to_eV = {'U0': 27.2114, 'U': 27.2114, 'G': 27.2114, 'H': 27.2114, 'zpve': 27211.4, 'gap': 27.2114, 'homo': 27.2114,
    ...
    dataloaders = {split: DataLoader(dataset,
                                     batch_size=batch_size,
                                     shuffle=args.shuffle if (split == 'strainall') else False,
                                     num_workers=num_workers,
                                     collate_fn=collate_fn)
                         for split, dataset in datasets.items()}

    # strain_dataloader = DataLoader(dataset,
    #                                  batch_size=batch_size,
    #                                  shuffle=args.shuffle if (split == 'train') else False,
    #                                  num_workers=num_workers,
    #                                  collate_fn=collate_fn)
