...
def random_split_for_self_training(dataset, task_idx=None, null_value=0,
                   frac_train=0.4, frac_valid=0.1, frac_test=0.1, frac_unlabeled=0.4, seed=0,
                 smiles_list=None):
    """
    ...
    [valid_smiles_list], [test_smiles_list])
    """
    np.testing.assert_almost_equal(frac_train + frac_valid + frac_test + frac_unlabeled, 1.0)
    ...
    # valid_idx = all_idx[int(frac_train * num_mols):int(frac_valid * num_mols)
    #                                                + int(frac_train * num_mols)]
    # test_idx = all_idx[int(frac_train * num_mols) + int(frac_valid * num_mols):int(frac_valid * num_mols)
    #                                                + int(frac_train * num_mols) + int(frac_test * num_mols)]

    # unlabeled_idx = all_idx[int(frac_valid * num_mols) + int(frac_train * num_mols) + int(frac_test * num_mols):]

    unlabeled_idx = all_idx[:int(frac_unlabeled * num_mols)]
    train_idx = all_idx[int(frac_unlabeled * num_mols):int(frac_train * num_mols) +
                        int(frac_unlabeled * num_mols)]

    valid_idx = all_idx[int(frac_unlabeled * num_mols) +
                        int(frac_train * num_mols):int(frac_unlabeled * num_mols) +
                        int(frac_train * num_mols) + int(frac_valid * num_mols)]

    test_idx = all_idx[int(frac_unlabeled * num_mols) + int(frac_train * num_mols) +
                       int(frac_valid * num_mols):]

    assert len(set(train_idx).intersection(set(valid_idx))) == 0
    assert len(set(valid_idx).intersection(set(test_idx))) == 0
    assert len(set(test_idx).intersection(set(unlabeled_idx))) == 0
    assert len(train_idx) + len(valid_idx) + len(test_idx) + len(unlabeled_idx) == num_mols

    train_dataset = dataset[torch.tensor(train_idx)]
    valid_dataset = dataset[torch.tensor(valid_idx)]
    test_dataset = dataset[torch.tensor(test_idx)]
    unlabeled_dataset = dataset[torch.tensor(unlabeled_idx)]

    if not smiles_list:
        return train_dataset, valid_dataset, test_dataset, unlabeled_dataset, train_idx
    else:
        train_smiles = [smiles_list[i] for i in train_idx]
        valid_smiles = [smiles_list[i] for i in valid_idx]
        test_smiles = [smiles_list[i] for i in test_idx]
        unlabeled_smiles = [smiles_list[i] for i in unlabeled_idx]
        return train_dataset, valid_dataset, test_dataset, unlabeled_dataset, (train_smiles,
                                                            valid_smiles,
                                                            test_smiles, unlabeled_smiles)
