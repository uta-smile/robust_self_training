...
def main():
    ...
    # input()
    iter_dict[0] = {'student_iteration': 0, 'best_epoch': best_epoch, 'best_val': best_val_score, 'best_test': test_score, 'model': copy.deepcopy(best_model), 'optimizer': best_opti_state}
    ...
    if args.self_training:
        ...
        if args.load_teacher:
            model = copy.deepcopy(iter_dict[0]['model'])
        else:
            model = GNN_graphpred(args.num_layer, args.emb_dim, num_tasks, JK = args.JK, drop_ratio = args.dropout_ratio, graph_pooling = args.graph_pooling, gnn_type = args.gnn_type)
            if not args.input_model_file == "":
                model.from_pretrained(args.input_model_file)
        ...
        if not args.use_unlabeled:
            ...
            for ite in range(1, args.st_iter+1):
                print('===========================runing student iteration ', ite)
                ...
                for epoch in range(1, args.s_epochs+1):
                    ...
                    # enumerate_epoch = epoch + ite * args.epochs
                    ...
                    train_loss_epoch = train(args, model, device, pseudo_dataset, optimizer, writer)
                    ...
                    if val_acc >= ite_best_val_score:
                        ...
                        ite_best_opti_state = optimizer.state_dict()
