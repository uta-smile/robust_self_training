...
def train(args, model, device, loader, optimizer, writer, loss_criterion, status='teacher'):
    ...
    if status == 'teacher':
        ...
        # for step, batch in enumerate(loader):
            ...
            elif not args.gjs_CE_loss and args.loss == 'JSWCS':
                # print('---------------using JSWCS')
                loss = loss_criterion(pred.double(), (y+1)/2)
                # input()
            else:
        ...
        if 'pseudo' in status:
            ...
            with open(os.path.join(output_dir_tmp + 'iter{}_preds.csv'.format(current_iter)), 'w') as f:
                ...
                if args.dataset == 'hiv':
                    ...
                    csv_writer.writerow(['smiles', 'p_np'])
                else:
                    print('check which dataset is used as input.')
...
def main():
    ...
    if not args.train_teacher:
        ...
        print('------- Start all the training from training teacher model...')
    ...
    total_score = {'train_score': train_acc_list, 'val_score': val_acc_list, 'test_score': test_acc_list}
