...
from model_condgated_cbe_aspp import GatedCBEASPP
from get_dataset import loadfasta, new_dataset, loadfasta_onehot, new_dataset_onehot
...
def main():
    ...
    if model_use == 'S4PRED':
        ...
        self_train_loader = DataLoader(loadfasta("self_train"), batch_size=batch_size, shuffle=False,
    ...
    elif model_use == 'Ensemble':
        test_loader = DataLoader(loadfasta_onehot("test"), batch_size=1, shuffle=False, num_workers=num_workers)
        valid_loader = DataLoader(loadfasta_onehot("valid"), batch_size=1, shuffle=False, num_workers=num_workers)
        self_train_loader = DataLoader(loadfasta_onehot("self_train"), batch_size=batch_size, shuffle=False,
                                       num_workers=num_workers)

        model = GatedCBEASPP(batch_size=batch_size, lstm_dropout_rate=dropout).to(device)
        # model = nn.DataParallel(model)
        pretrain_model = GatedCBEASPP(batch_size=batch_size).to(device)
    else:
        return "Unknown model choose "
    ...
    for i in range(i_num, iteration):
        ...
        # optimizer = optim.Adam(model.parameters(), lr=learn_rate, betas=(0.9, 0.999))
        print(f'Iteration  {i} ')
        print('Get predicted data', datetime.datetime.now() - start)
        get_predict(self_train_loader, i, pretrain_model)
        print('Loading new dataset', datetime.datetime.now() - start)
        if model_use == 'S4PRED':
            iteration_dataset = new_dataset(os.path.join(data_path, str(i)))
        elif model_use == 'Ensemble':
            iteration_dataset = new_dataset_onehot(os.path.join(data_path, str(i)))
        else:
            return "Unknown model choose "

        iteration_loader = DataLoader(iteration_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
